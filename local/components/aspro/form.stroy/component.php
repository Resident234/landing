...
\Bitrix\Main\Diag\Debug::writeToFile($_REQUEST, "PERSONAL", "component.log");
...
if (CModule::IncludeModule("iblock")) {
    ...
    $bCache = !($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["form_submit"]) || $_REQUEST['formresult'] == 'ADDOK') && !($arParams["CACHE_TYPE"] == "N" || ($arParams["CACHE_TYPE"] == "A" && COption::GetOptionString("main",
                    "component_cache_on",
                    "Y") == "N") || ($arParams["CACHE_TYPE"] == "Y" && intval($arParams["CACHE_TIME"]) <= 0));
    ...
    $arMess = CEventMessage::GetList($arResult["SITE"]["ID"], $order = "desc",
        array("TYPE_ID" => $event_name))->Fetch();
    if (!is_array($arMess)) {
    ...
    $arMessAdmin = CEventMessage::GetList($arResult["SITE"]["ID"], $order = "desc",
        array("TYPE_ID" => $event_name_admin))->Fetch();
        ...
        foreach ($arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion) {
            if (isset($_REQUEST[$FIELD_CODE])) {
                ...
                if ($arQuestion["MULTIPLE"] == "Y") {
                    $valFile = array(
                        'n0' => $arQuestion["DEFAULT"],
                        'n1' => $arQuestion["DEFAULT"],
                        'n2' => $arQuestion["DEFAULT"],
                        'n3' => $arQuestion["DEFAULT"]
                    );
                }
            }
            ...
            switch ($arQuestion["FIELD_TYPE"]) {
                ...
                case "html":
                    ...
                    break;
                case "list":
                    $rsValue = CIBlockProperty::GetPropertyEnum($arQuestion["CODE"],
                        array("SORT" => "ASC", "ID" => "ASC"), array("IBLOCK_ID" => $arParams["IBLOCK_ID"]));
                    ...
                    while ($arValue = $rsValue->Fetch()) {
                        $selected = '';
                        if (!empty($val) && (!is_array($val) ? ($arValue["ID"] == $val) : (in_array($arValue["ID"],
                                $val)))) {
    ...
    if (strlen($arResult["ERROR"]) <= 0) {
        ...
        if (strlen($_REQUEST["form_submit"]) > 0 && ($_REQUEST['FORM_ID'] == $arResult['FORM_ID'])) {




            if (isset($_REQUEST["PERSONAL"]) && $arParams["PERSONAL_CHECKBOX"]["ENABLED"] == "Y"){
                if(!$_REQUEST["PERSONAL"]) {
                    $arResult["FORM_ERRORS"][] = $arParams["PERSONAL_CHECKBOX"]["NOTICE"];
                };
            };
            ...
            if ($arParams["USE_CAPTCHA"] == "Y" && (empty($_REQUEST["captcha_word"]) || !$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word"],
                ...
                foreach ($_REQUEST as $code => $value) {
                    if ($arResult["QUESTIONS"][$code]["FIELD_TYPE"] == "html") {
                        $_REQUEST[$code] = array(
                            "VALUE" => array(
                                "TEXT" => $value,
                                "TYPE" => $arResult["QUESTIONS"][$code]["FIELD_TYPE"]
                            )
                        );
                        ...
                        $_REQUEST[$code] = array(
                            "VALUE" => str_replace(array('-', '/', ' ', ':'), array('.', '.', '.', '.'),
                                    $arDateTime[0]) . ' ' . str_replace(array(
                                    '-',
                                    '/',
                                    ' ',
                                    ':'
                        ...
                        foreach ($arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion) {
                            ...
                            if ($arQuestion["FIELD_TYPE"] == "list") {
                                ...
                                $dbRes = CIBlockElement::GetList(array(),
                                    array('ID' => $RESULT_ID, 'IBLOCK_ID' => $arParams['IBLOCK_ID']), false, false,
                                    array('ID', 'PROPERTY_' . $FIELD_CODE));
        ...
        $arResult = array_merge(
            ...
            array(
                ...
                "FORM_HEADER" => sprintf(
                        "<form name=\"%s\" action=\"%s\" method=\"%s\" class='js-form js-form__%s' enctype=\"multipart/form-data\">",
                        $arResult["IBLOCK_CODE"], POST_FORM_ACTION_URI, "POST", $arResult["FORM_ID"]
                    ) . $res .= bitrix_sessid_post() . '<input type="hidden" name="FORM_ID" value="'.$arResult['FORM_ID'].'" />',
